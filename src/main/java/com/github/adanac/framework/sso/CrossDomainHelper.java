 ...
 */
package com.github.adanac.framework.sso;
...
import com.github.adanac.framework.sso.common.CookieHelper;
import com.github.adanac.framework.sso.common.encrypt.AES;
import com.github.adanac.framework.sso.common.util.ReflectUtil;
...
public class CrossDomainHelper {
	 ...
	 * 				AES 密钥
	 */
	public String askCiphertext(AuthToken authToken, String aesKey) {
		try {
	 ...
	 * 				AES 密钥
	 */
	public String replyCiphertext(HttpServletRequest request, String userId,
	...
	public String ok(HttpServletRequest request, HttpServletResponse response,
			String replyTxt, String publicKey, String aesKey) {
		AuthToken authToken = getAuthToken(request, publicKey);
		if (authToken != null) {
			String rt = null;
			try {
			...
			if (rt != null) {
				...
				if (atk != null && atk.getUserIp().equals(authToken.getUserIp())) {
					if (atk.verify(publicKey) != null) {
						/**
						 * 删除跨域信任Cookie
						 * 返回 userId
						 */
